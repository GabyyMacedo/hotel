...
function Cadastro() {
    ...
    if (lista_clientes.some(cliente => cliente.id === identidade && cliente.senha === senha)) {
        ...
        for (let i = 1; i < (parseInt((document.getElementById("acompanhantes"))) + 1); i++) {
            let dep = [];
            dep.push(identidade);
            dep.push(`${i} acompanhantes`);
...
function Login(){
    ...
    let senha = document.getElementById("Senha").value
    let identidade = Encontrar_pessoa_pelo_nome(nome)
    if (lista_clientes.some(cliente => cliente.id === identidade && cliente.senha === senha) ) {
...
function mostrarindex2(paginaId){
